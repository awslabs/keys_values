...
def test_smart_lastrec_update_protected_ranges(
    ...
    for _ in range(num_repeats):
        ...
        input_ids = torch.cat(
            [
                torch.cat((torch.zeros(nlp, **kwargs), x[0:1], y, x[1:])).unsqueeze(0)
        ...
        assert _expected_start == cache.protected_start, (
            _expected_start,
            cache.protected_start,
        )
        assert _expected_end == cache.protected_end, (
            _expected_end,
            cache.protected_end,
        )
...
def test_smart_lastrec_update_ranges_several_chunks(
    ...
    for shift_chunks, tokens, nlp, el, rn in zip(
        ...
        if shift_chunks == 0:
            ...
            fill_sz = chunk_size
            fill_right = bogus_token.expand(fill_sz, -1).flatten()
            new_tokens = torch.cat((pad_left, tokens[:-1], fill_right, tokens[-1:]))
            expected_start.append(e_start)
            expected_end.append(e_end)
    ...
    assert expected_start == cache.protected_start, (
        expected_start,
        cache.protected_start,
    )
