...
def test_fused_rmsnorm_forward(batch_dims, D, dtype, add_unit_offset):
    ...
    references = {
        ...
        "kv_model.RMSNorm": _rmsnorm_via_module(
            kv_model.RMSNorm, x, weight, eps, add_unit_offset
        ),
        "litgpt.RMSNorm": _rmsnorm_via_module(
            litgpt_model.RMSNorm, x, weight, eps, add_unit_offset
        ),
    }
...
def test_fused_rmsnorm_backward(batch_dims, D, dtype, add_unit_offset):
    ...
    references = {
        ...
        "kv_model.RMSNorm": _run(
            lambda x, w: _rmsnorm_via_module(
                kv_model.RMSNorm, x, w, eps, add_unit_offset
            )
        ),
        "litgpt.RMSNorm": _run(
            lambda x, w: _rmsnorm_via_module(
                litgpt_model.RMSNorm, x, w, eps, add_unit_offset
            )
        ),
    }
...
_ROPE_PARAMS_3D = [
    (BH, T, D, dtype) for (BH, T, D), dtype in product(_ROPE_SHAPES_3D, _ROPE_DTYPES)
]
_ROPE_PARAMS_4D = [
    (B, n_head, T, D, dtype)
...
def _rope_eager(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
...
def test_fused_apply_rope_nd_forward(B, n_head, T, D, dtype):
    ...
    for name, out_ref in references.items():
        print(
            f"rope fwd 4D: B={B}, n_head={n_head}, T={T}, D={D}, dtype={dtype}, ref={name}"
        )
        torch.testing.assert_close(out_fused, out_ref, atol=atol, rtol=rtol)
...
def test_fused_swiglu_llamamlp_forward(shape_cfg, dtype):
    ...
    for mlp_class, cfg, label in (
        ...
        print(
            f"swiglu mlp fwd: shape={(*batch_shape, n_embd)}, dtype={dtype}, ref={label}"
        )
