...
class FlashInferSDPA:
    ...
    def _flashinfer_sdpa(
        ...
        if use_decode_kernel:
            ...
            # Two-phase: FlashInfer prefill for O+LSE, then compute weights from LSE
    ...
    def _flashinfer_sdpa_standard(
        ...
        # The kernel now supports per-head token positions
        # Input shape: (batch_size, n_query_groups, kv_len)
        # Output shape for kernel: (batch_size, n_query_groups, kv_len) - passed directly
        token_positions_transformed = None
        if token_positions is not None:
            # Pass the full token_positions tensor - kernel handles per-head positions
