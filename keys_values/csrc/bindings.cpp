...
/**
 * @brief Fused Scaled Dot Product Attention for prefill phase
 *
 * Same interface as sdpa_prefill but uses the fused kernel that accumulates
 * attention weights during the tiling loop, avoiding the large intermediate
 * [q_len, num_qo_heads, kv_len] tensor and separate accumulation pass.
 *
 * When return_weights=True, this is significantly more memory-efficient and
 * faster than the two-pass approach in sdpa_prefill.
 */
std::tuple<torch::Tensor, std::optional<torch::Tensor>, std::optional<torch::Tensor>> sdpa_prefill_fused(
    const torch::Tensor& query,
    const torch::Tensor& key,
    const torch::Tensor& value,
    double scale,
    ...
    TORCH_CHECK(query.scalar_type() == key.scalar_type(), "query and key must have same dtype");
    TORCH_CHECK(key.scalar_type() == value.scalar_type(), "key and value must have same dtype");
    ...
    // If no weights needed or return_lse mode, delegate to original sdpa_prefill
    if (!return_weights || return_lse) {
        return sdpa_prefill(query, key, value, scale, token_positions, input_pos,
                           sliding_window_size, causal, return_weights, return_lse);
    }

    // Score cache: caches raw Q·K*scale scores during the tiling loop to avoid K re-reads.
    // This gives ~1.01x overhead (vs 1.9x for old two-pass) when q_len ≈ kv_len.
    // When the cache would be too large, delegate to old two-pass sdpa_prefill which
    // has low overhead at large kv_len anyway (1.06-1.10x at 128K).
    // Score cache is beneficial when q_len is comparable to kv_len (q_len >= kv_len/8).
    // When q_len << kv_len, the score cache bandwidth dominates and old approach is better.
    constexpr int64_t SCORE_CACHE_MAX_BYTES = 2LL * 1024 * 1024 * 1024;  // 2GB
    int64_t score_cache_bytes = q_len * num_qo_heads * kv_len * static_cast<int64_t>(sizeof(float));
    bool use_score_cache = (score_cache_bytes <= SCORE_CACHE_MAX_BYTES) && (q_len * 8 >= kv_len);

    if (!use_score_cache) {
        // Delegate to old two-pass approach — better than fused fallback at large kv_len
        return sdpa_prefill(query, key, value, scale, token_positions, input_pos,
                           sliding_window_size, causal, return_weights, return_lse);
    }

    // Allocate summed weights directly — NO per-query intermediate
    auto weights_options = torch::TensorOptions().dtype(torch::kFloat32).device(query.device());
    torch::Tensor attn_weights_sum;
    if (batched) {
        attn_weights_sum = torch::zeros({batch_size, num_kv_heads, kv_len}, weights_options);
    } else {
        attn_weights_sum = torch::zeros({num_kv_heads, kv_len}, weights_options);
    }

    torch::Tensor score_cache;
    if (batched) {
        score_cache = torch::empty({batch_size, q_len * num_qo_heads * kv_len}, weights_options);
    } else {
        score_cache = torch::empty({q_len * num_qo_heads * kv_len}, weights_options);
    }
    ...
    if (batched) {
        if (dtype == at::ScalarType::Half) {
            ...
            params.input_pos = input_pos.has_value() ?
                input_pos.value().data_ptr<int32_t>() : nullptr;
            params.sliding_window_size = static_cast<int32_t>(sliding_window_size);
            params.return_attn_weights = false;
            params.causal = causal;

            err = kernels::launch_batch_fused_prefill_attention(
                params, attn_weights_sum.data_ptr<float>(),
                score_cache.data_ptr<float>(), stream);
        } else if (dtype == at::ScalarType::BFloat16) {
            kernels::BatchPrefillParams<__nv_bfloat16, __nv_bfloat16, __nv_bfloat16> params;
            params.q = reinterpret_cast<const __nv_bfloat16*>(query.data_ptr());
            params.k = reinterpret_cast<const __nv_bfloat16*>(key.data_ptr());
        ...
        // Single (non-batched) fused prefill
        int32_t input_pos_val = 0;
        if (input_pos.has_value()) {
            TORCH_CHECK(input_pos.value().numel() == 1, "input_pos must be scalar for non-batched input");
            input_pos_val = input_pos.value().item<int32_t>();
        }
        ...
        if (dtype == at::ScalarType::Half) {
            ...
            params.token_positions = token_positions.has_value() ?
                token_positions.value().data_ptr<int32_t>() : nullptr;
            params.token_positions_stride_h = (token_positions.has_value() && token_positions_per_head) ?
                static_cast<uint32_t>(token_positions.value().stride(0)) : 0;
            params.input_pos = input_pos_val;
            params.sliding_window_size = static_cast<int32_t>(sliding_window_size);
            params.return_attn_weights = false;
            params.causal = causal;
            err = kernels::launch_single_fused_prefill_attention(
                params, attn_weights_sum.data_ptr<float>(),
                score_cache.data_ptr<float>(), stream);
        } else if (dtype == at::ScalarType::BFloat16) {
            kernels::SinglePrefillParams<__nv_bfloat16, __nv_bfloat16, __nv_bfloat16> params;
    ...
    std::optional<torch::Tensor> weights_opt = attn_weights_sum;
    return std::make_tuple(output, weights_opt, lse_output);
}


/**
 * @brief Fused v2 Scaled Dot Product Attention for prefill phase
 *
 ...
 */
std::tuple<torch::Tensor, std::optional<torch::Tensor>, std::optional<torch::Tensor>> sdpa_prefill_fused_v2(
    const torch::Tensor& query,
    const torch::Tensor& key,
    const torch::Tensor& value,
    double scale,
    ...
    // Allocate summed weights — NO per-query intermediate, NO score cache
    auto weights_options = torch::TensorOptions().dtype(torch::kFloat32).device(query.device());
    torch::Tensor attn_weights_sum;
    if (batched) {
        attn_weights_sum = torch::zeros({batch_size, num_kv_heads, kv_len}, weights_options);
    ...
    if (batched) {
        if (dtype == at::ScalarType::Half) {
            ...
            err = kernels::launch_batch_fused_prefill_attention_v2(
                params, attn_weights_sum.data_ptr<float>(), stream);
        } else if (dtype == at::ScalarType::BFloat16) {
            kernels::BatchPrefillParams<__nv_bfloat16, __nv_bfloat16, __nv_bfloat16> params;
            params.q = reinterpret_cast<const __nv_bfloat16*>(query.data_ptr());
            params.k = reinterpret_cast<const __nv_bfloat16*>(key.data_ptr());
        ...
        // Single (non-batched) fused prefill v2
        int32_t input_pos_val = 0;
        if (input_pos.has_value()) {
            TORCH_CHECK(input_pos.value().numel() == 1, "input_pos must be scalar for non-batched input");
            input_pos_val = input_pos.value().item<int32_t>();
        ...
        if (dtype == at::ScalarType::Half) {
            ...
            params.return_attn_weights = false;
            params.causal = causal;
            err = kernels::launch_single_fused_prefill_attention_v2(
                params, attn_weights_sum.data_ptr<float>(), stream);
        } else if (dtype == at::ScalarType::BFloat16) {
            kernels::SinglePrefillParams<__nv_bfloat16, __nv_bfloat16, __nv_bfloat16> params;
            params.q = reinterpret_cast<const __nv_bfloat16*>(query.data_ptr());
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("sdpa_prefill_fused_v2", &keys_values::bindings::sdpa_prefill_fused_v2,
        R"doc(
    ...
    m.def("sdpa_prefill_fused", &keys_values::bindings::sdpa_prefill_fused,
        R"doc(
        ...
        attention weights during the tiling loop. Avoids allocating the large
        [q_len, num_qo_heads, kv_len] intermediate and the separate accumulation pass.
