...
def create_chunk_sizes(
    ...
    if seq_length <= mpl:
        ...
        while num_done < seq_length:
            ...
            num_done += c_size
    assert sum(chunk_sizes) == seq_length, (
        sum(chunk_sizes),
        seq_length,
        chunk_sizes,
    )  # Sanity check
    ...
    * Why not outer over layers, inner over chunks? This would require to
    ...
    def _select_chunks_and_cells(
        ...
        chunk_sizes = create_chunk_sizes(
            ...
            num_output_tokens=(
                0
                if self.single_tokens_for_targets or not inputs_targets_boundary
                else num_output_tokens
            ),
        )
        assert all(
        ...
        if self._debug_single_cell_per_row:
            ...
            if not inputs_targets_boundary:
                ...
                chunks_per_cell = [
                    len(chunk_sizes) - num_target_chunks,
                    num_target_chunks,
                ]
        else:
            factor = (
            ...
            for cs_part in parts:
                ...
                for chunk_size in cs_part:
                    ...
                    if new_length > max_cell_length and num_chunks > 0:
                        ...
                        num_chunks += 1
                this_chunks_per_cell.append(num_chunks)
                assert sum(this_chunks_per_cell) == len(cs_part), (
                    sum(this_chunks_per_cell),
                    len(cs_part),
                    this_chunks_per_cell,
                )
