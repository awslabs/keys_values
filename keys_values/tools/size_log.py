...
class StoreWeightsRule:
    ...
    def combine(
        self, parts: List[Tuple[str, int, torch.Tensor]]
        ...
        if self.num_layers is not None and inds != tuple(range(0, self.num_layers)):
            raise ValueError(
                f"StoreWeightsRule[{self.name}]: inds = {inds}, must be equal to 0:{self.num_layers}"
            )
...
class SizeWeightsGradientsLog:
    ...
    def __init__(
        ...
        self._name_pos: Dict[str, Optional[Dict[str, int]]] = {
            k: None for k in ENTRIES_KEYS
        }
        self.mapper = mapper
        self.store_rules = {
        ...
        for k in ENTRIES_KEYS:
            ...
            if new_stored[k]:
                ...
                if num_new > 0:
                    ...
                    print(
                        f"Append {num_new} entries to dictionary stored to {store_path}"
                    )
...
def get_match_for_store_rule(postfix: str) -> Callable[[str], Optional[int]]:
    regex = re.compile(r"transformer\.h\.(\d+)\." + re.escape(postfix) + r"$")
