    ...
    def __call__(self, x: torch.Tensor):
        if self.file_name is not None:
            ...
            if torch.isnan(x_det).any():
                ...
                raise AssertionError(f"Tensor: {torch.isnan(x_det).sum()} of {x_det.numel()} entries are NaN")
            if self.threshold is not None:
                if (torch.abs(x_det) > self.threshold).any():
                    torch.save(x_det, self.file_name)
                    raise AssertionError(f"Tensor: No NaNs, but {(torch.abs(x_det) > self.threshold).sum()} of {x_det.numel()} entries are |elem| > {self.threshold}")
        return x
...
def initialize_weights_tracker(
    file_name: str, threshold: Optional[float] = None,
):
    """
    ...
    Args:
        ...
        threshold: If given, we don't just check for NaNs, but also for entries
            with absolute value larger then this.
