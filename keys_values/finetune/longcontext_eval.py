...
def main(
    ...
    with fabric.init_module(empty_init=(fabric.world_size > 1)):
        ...
        if is_lora:
            mark_only_lora_as_trainable(
                gpt_model,
                lora_kind=model_config.config.lora_kind,
            )
    ...
    for batch in test_dataiter:
        ...
        try:
            ...
            if evaluator is None:
                ...
                with torch.no_grad():
                    metrics_values["val_loss"] = model(input_ids, targets, mode="targets")
            eval_time = time.perf_counter() - t0
            msg = ", ".join(f"{k} = {v.mean().item():.3f}" for k, v in metrics_values.items())
            print_with_rank_and_timestamp(
