...
from keys_values.kvcache.gradient.checkpoints import (
    ...
    FileBasedQuantizerStateForCheckpoint,
)
...
class LongContextGradientModel(LongContextInferenceModel):
    ...
    * For evaluation, use :meth:`copy_model_for_evaluation` to obtain a
    ...
    def __init__(
        ...
        Args:
            ...
            cachecp_pin_memory: If `True`, the CPU memory pages for KV cache
                checkpoints are pinned. This can run faster, but also needs more
                real CPU memory.
            checkpoint_name_manager: If this is given, we use file-based storage
                of checkpoints eventually, once CPU memory is occupied. The names
                of storage files are generated by this object, which also does
                the cleanup.
            checkpoint_frac_ram: Only used if `checkpoint_name_manager` is given.
                See `keys_values.finetune.args.GradientArgs.checkpoint_frac_ram`.
                We determine the threshold value here, so that at least a fraction
                of `checkpoint_frac_ram` is not used for storing checkpoints.
        ...
        if checkpoint_name_manager is not None and checkpoint_frac_ram is None:
            raise ValueError(
    ...
    def _create_layer_checkpointers(self, seq_length: int):
        """
        If an external volume memory manager is used to store checkpoints to
        disk eventually, we set its `threshold` parameter here. The idea is to
        protect a fraction `checkpoint_frac_ram` of CPU RAM currently available
        from being used for checkpoints. If this is not done, the program may
        crash or freeze. This can also happen if `checkpoint_frac_ram` is too
        small.
        ...
        if use_filebased_memory and self.layercp_qname != "default":
