...
class GradientAccumulator:
    ...
    * Forward-backward computations over columns, from right to left, using
    ...
    def _select_cache_buffers(
        self, cache_lengths_of_shard: List[int],
    ...
    def _get_checkpoints_and_buffers(
        ...
        cache_lengths = [kv_cache.cache_length for _, kv_cache in model_part.get_kv_caches()]
    ...
    def _deallocate_buffers(self):
        for buffer in (buffer for buffers in self._buffers_per_length.values() for buffer in buffers):
            buffer.deallocate()
