...
def extract_index(
    ...
    **index_kwargs,
    ...
    max_one_step = cache_length - max(
        b - a if a is not None else 0 for a, b in zip(protected_start, protected_end)
    )
    if num <= max_one_step:
        offs_and_nums = ((0, num),)
    else:
        ...
        assert (
            num <= 2 * max_one_step
        ), f"Internal error: num = {num}, max_one_step = {max_one_step}"
        offs_and_nums = (
            (0, max_one_step),
...
class SmartInitialLastRecentlyInsertedKVCacheReplayLog(DefaultKVCacheReplayLog):
    ...
    def __init__(
        ...
        self.pad_id = pad_id
        self.range_is_prefix = range_is_prefix
    ...
    def extract_index(
        ...
        if next_position is None:
            raise IndexError(
                f"input_pos = {input_pos} not represented (keys are {list(self.next_position_for_input_pos.keys())})"
            )
...
class SmartInitialLastRecentlyInsertedKVCache(KVCacheWithBuffers):
    ...
    * Chunk length `key.shape[2]`
    ...
    If `range_is_prefix == True`, protected ranges are prefixes, so
    `protected_start[b] = 0` and they are always determined from the prefill
    chunk. In this case, protected ranges may contain left-padding tokens.

    TODO: This option is switched on by default, since we ran experiments with
    `range_is_prefix == False` and did not see improvements. To be revisited!
    ...
    def from_config(
        ...
        **base_kwargs,
        ...
        return SmartInitialLastRecentlyInsertedKVCache(
            ...
            pad_id=pad_id,
            range_is_prefix=range_is_prefix,
            **base_kwargs,
        )
    ...
    def _prefill_internal(
        ...
        if self._replay_log is not None:
            self._replay_log = SmartInitialLastRecentlyInsertedKVCacheReplayLog(
                ...
                pad_id=self._pad_id,
                range_is_prefix=self.range_is_prefix,
                protected_start=self.protected_start,
                protected_end=self.protected_end,
            )
