...
from litgpt.lora import (
    mark_only_lora_as_trainable as litgpt_mark_only_lora_as_trainable,
)
...
class Block(BaseBlock):
    ...
    def reset_lora_parameters(self) -> None:
        self.attn.reset_lora_parameters()
        self.mlp.reset_lora_parameters()
...
class GptNeoxMLP(litgpt.model.GptNeoxMLP):
    def __init__(self, config: Config) -> None:
        nn.Module.__init__(self)
        self.fc = create_lora_or_dora_linear(
            config, config.n_embd, config.intermediate_size
        )
        self.proj = create_lora_or_dora_linear(
...
class LLaMAMLP(litgpt.model.LLaMAMLP):
    def __init__(self, config: Config, intermediate_size: Optional[int] = None) -> None:
        ...
        self.fc_1 = create_lora_or_dora_linear(
            config, config.n_embd, self.intermediate_size
        )
        self.fc_2 = create_lora_or_dora_linear(
            config, config.n_embd, self.intermediate_size
        )
        self.proj = create_lora_or_dora_linear(
            config, self.intermediate_size, config.n_embd
        )
        self.config = config
...
class LLaMAMoE(litgpt.model.LLaMAMoE):
    def __init__(self, config: Config) -> None:
        nn.Module.__init__(self)
        self.gate = create_lora_or_dora_linear(
