...
def batched_generate_fn(
    ...
    *,
    ...
    if not is_logits:
        ...
        assert prompts_or_logits.ndim == 2, "prompts_or_logits must be a 2D or 3D tensor."
        batch_size, max_prompt_size = prompts_or_logits.shape
    else:
        if include_prompt:
            raise ValueError("include_prompts=True invalid if prompts_or_logits are logits")
    ...
    if not is_logits:
        ...
        # We need the logits for the final chunk in order to generate the
        # first token below. Chunk size does not matter, just must be nonzero.
        logits_final_chunk = model(prompts_or_logits, targets=None)
    else:
        logits_final_chunk = prompts_or_logits
