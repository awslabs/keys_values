    ...
    def report(self) -> str:
        parts = [
            ...
            for args, num_hits in sorted(
                self.num_hits.items(), key=lambda x: x[1], reverse=True,
            )
        ]
        return "\n".join(parts)
...
class FlexAttnForChunkManager(FlexAttnManager):
    ...
    def _create_block_mask(
        ...
        **kwargs,
        ...
        if q_len > kv_len:
            raise ValueError(f"q_len={q_len}, kv_len={kv_len}: Must have q_len <= kv_len")
        mask_mod = partial(
...
def choose_q_lens(
    ...
    if num_q_lens < chunk_size:
        q_lens = [math.ceil(i * chunk_size / num_q_lens) for i in range(1, num_q_lens + 1)]
