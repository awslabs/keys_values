...
class SampleBasedMetricsEvaluator:
    ...
    def supported_metrics(old_setup: bool = False) -> List[str]:
        ...
        """
        return list(
            METRICS_FOR_HELMET_TASKS.values()
            if not old_setup
            else METRICS_FOR_HELMET_TASKS_OLD.values()
        )
    ...
    def metric_for_helmet_task(
        ...
        """
        return (
            METRICS_FOR_HELMET_TASKS.get(dataset_key)
            if not old_setup
            else METRICS_FOR_HELMET_TASKS_OLD.get(dataset_key)
        )
